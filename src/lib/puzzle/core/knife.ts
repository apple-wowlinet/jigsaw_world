...
import type { CtrlPt, Edge, Piece, PieceChoice, Template } from './types';
...
function mkTemplate(
  ...
  return {
    name,
    pts: pts.map((p): CtrlPt => ({ fromBase: p.f, alongBase: p.a })),
    ptsReversed: rev.map((p): CtrlPt => ({ fromBase: p.f, alongBase: p.a })),
  };
}
...
export function drawEdge(c: CanvasRenderingContext2D, e: Edge) {
  if (e.border) {
