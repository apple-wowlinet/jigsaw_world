 ...
 *  3. 存档序列化↔恢复幂等
 */
const knife_1 = require("../src/lib/puzzle/core/knife");
const scatter_1 = require("../src/lib/puzzle/core/scatter");
const group_1 = require("../src/lib/puzzle/core/group");
let failures = 0;
...
function mkSubject(w, h) {
    return { canvas: null, width: w, height: h };
}
function choiceFor(subW, subH, targetNop) {
...
console.log('1. 切图确定性');
{
    ...
    const a = (0, knife_1.buildPieces)(sub, choice, 12345);
    const b = (0, knife_1.buildPieces)(sub, choice, 12345);
    assert(a.length === b.length, 'piece count mismatch');
    for (let i = 0; i < a.length; i++) {
        const ea = a[i].edges, eb = b[i].edges;
        for (const side of ['top', 'right', 'bottom', 'left']) {
            assert(ea[side].tab === eb[side].tab &&
                ea[side].bend === eb[side].bend &&
                ea[side].border === eb[side].border &&
                (ea[side].curves?.name ?? '') === (eb[side].curves?.name ?? ''), `edge mismatch piece ${i} ${side}`);
        }
        assert(a[i].width === b[i].width && a[i].height === b[i].height, `size mismatch piece ${i}`);
    }
    const c = (0, knife_1.buildPieces)(sub, choice, 54321);
    let diff = 0;
    for (let i = 0; i < a.length; i++) {
        for (const side of ['top', 'right', 'bottom', 'left']) {
            if (a[i].edges[side].tab !== c[i].edges[side].tab ||
...
for (const board of boards) {
    for (const targetNop of nops) {
        ...
        if (choice.size < 8)
            continue; // 块太小无意义
        const pieces = (0, knife_1.buildPieces)(sub, choice, 999);
        for (const p of pieces) {
            p.group = new group_1.Group([p]);
        }
        const world = (0, scatter_1.ensureScatterCapacity)(board.W, board.H, subW, subH, pieces);
        (0, scatter_1.scatter)({
            pieces,
            fixed: [],
            W: world.W,
        ...
        for (const p of pieces) {
            ...
            if (b.x + b.w > prev.left && b.x < prev.right &&
        ...
        for (let i = 0; i < pieces.length; i++) {
            const a = (0, group_1.pieceBounds)(pieces[i]);
            for (let j = i + 1; j < pieces.length; j++) {
                const b = (0, group_1.pieceBounds)(pieces[j]);
                const ox = Math.min(a.x + a.w - MARGIN, b.x + b.w - MARGIN) -
                    Math.max(a.x + MARGIN, b.x + MARGIN);
                const oy = Math.min(a.y + a.h - MARGIN, b.y + b.h - MARGIN) -
...
console.log('3. 归一化坐标往返精度');
{
    const sub = mkSubject(600, 400);
    const choice = choiceFor(600, 400, 24);
    const pieces = (0, knife_1.buildPieces)(sub, choice, 777);
    const W = 1400, H = 900;
    for (const p of pieces) {
        p.x = Math.random() * W;
        p.y = Math.random() * H;
