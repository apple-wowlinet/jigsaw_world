...
exports.ensureScatterCapacity = ensureScatterCapacity;
exports.capacityForBoxes = capacityForBoxes;
...
function boundsInfo(p) {
    const b = (0, group_1.pieceBounds)(p);
    return {
        w: b.w,
        h: b.h,
        offX: b.x + b.w / 2 - p.x,
...
const shrink = (v) => Math.max(4, v - constants_1.MARGIN + 1);
/**
 * 网格参数。格距 = 最大块碰撞盒 + 2px：相邻格心距离 ≥ 任意两块
 * 碰撞半宽之和，同格不复用 ⇒ 零重叠结构保证。
 * avg 仅用于保护区外扩与抖动幅度（对齐 jigex 语义）。
 */
function gridOf(W, H, pieces) {
    let maxCW = 0, maxCH = 0, sumW = 0, sumH = 0;
...
/** 中心保护区：参考图矩形外扩 avg/4 */
function guardOf(W, H, subjectW, subjectH, avgW, avgH) {
    return {
...
function spiralCells(W, H, g, guard) {
    const { cols, rows, stepW, stepH } = g;
    const cells = [];
    const cellFree = (x, y) => !(x + stepW / 2 > guard.left &&
 ...
 */
function ensureScatterCapacity(boardW, boardH, subjectW, subjectH, pieces) {
    let W = boardW, H = boardH;
    for (let i = 0; i < 64; i++) {
        const g = gridOf(W, H, pieces);
        const guard = guardOf(W, H, subjectW, subjectH, g.avgW, g.avgH);
        const cells = spiralCells(W, H, g, guard);
        if (cells.length >= pieces.length * 1.3)
            break;
        W *= 1.08;
        H *= 1.08;
    }
 ...
 */
function capacityForBoxes(boardW, boardH, subjectW, subjectH, boxW, boxH, count) {
    const needW = Math.max(8, boxW - constants_1.MARGIN * 2) + 2;
    const needH = Math.max(8, boxH - constants_1.MARGIN * 2) + 2;
    let W = boardW, H = boardH;
    for (let i = 0; i < 64; i++) {
        const cols = Math.max(1, Math.floor(W / needW));
        const rows = Math.max(1, Math.floor(H / needH));
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const x = (c + 0.5) * stepW;
                const y = (r + 0.5) * stepH;
                const hits = x + stepW / 2 > guard.left &&
                    x - stepW / 2 < guard.right &&
                    y + stepH / 2 > guard.top &&
                    y - stepH / 2 < guard.bottom;
                if (!hits)
...
function scatter(params) {
    ...
    const cells = spiralCells(W, H, g, guard);
    const intersectsGuard = (bcx, bcy, hw, hh) => bcx + hw > guard.left &&
        bcx - hw < guard.right &&
        bcy + hh > guard.top &&
        bcy - hh < guard.bottom;
    ...
    const nearNeighbor = (p, ax, ay) => {
        for (const n of p.neighbors) {
            const pos = placedPos.get(n.id);
            if (!pos)
                continue;
            const joinDist = utils_1.Utils.hypot(p.refX - n.refX, p.refY - n.refY) * 1.5;
            if (utils_1.Utils.hypot(ax - pos.x, ay - pos.y) < joinDist)
                return true;
        }
        return false;
    };
    ...
    for (const p of order) {
        ...
        const hw = bi.w / 2, hh = bi.h / 2;
        const shw = shrink(hw), shh = shrink(hh);
        let done = false;
        for (const avoidNeighbor of [true, false]) {
            for (const cell of cells) {
                ...
                const bx = utils_1.Utils.clamp(cell.x, hw, W - hw);
                const by = utils_1.Utils.clamp(cell.y, hh, H - hh);
                if (intersectsGuard(bx, by, hw, hh))
                    continue;
                if (collides(bx, by, shw, shh))
                    continue;
                ...
                const ax = utils_1.Utils.clamp(bx + jx, hw, W - hw);
                const ay = utils_1.Utils.clamp(by + jy, hh, H - hh);
                if (!intersectsGuard(ax, ay, hw, hh) &&
        ...
        for (const allowOverlap of [false, true]) {
            ...
            while (!spot) {
                ...
                if (intersectsGuard(x, y, hw, hh))
                    continue;
                if (!allowOverlap && collides(x, y, shrink(hw), shrink(hh)))
                    continue;
                spot = { x, y };
            }
            if (spot)
        ...
        if (spot) {
            ...
            commit(p, utils_1.Utils.clamp(hw, hw, W - hw), utils_1.Utils.clamp(hh, hh, H - hh), bi);
        }
    }
}
